...
use Koiterm\Base\BaseInterface;
use Koiterm\Inc\CommonFunc;

class Table extends BaseInterface
{
    ...
    public function __construct($para = array()) {
        ...
        if(isset($this->_pre_cache_key) && (($ttl = CommonFunc::getglobal('setting/memory/'.$this->_table)) !== null || ($ttl = $this->_cache_ttl) !== null) && memory('check')) {
            $this->_cache_ttl = $ttl;
            $this->_allowmem = true;
        }
        $this->_init_extend();
